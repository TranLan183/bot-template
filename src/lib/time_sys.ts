...
class TimeSystem {
    ...
    getDateInFuture(current: Date, options?: {
        ...
        return new Date(Number(BigInt(current.getTime()) + BigInt(next_seconds * MILLISECOND_PER_ONE_SEC + next_minutes * MILLISECOND_PER_ONE_SEC * 60 + next_hours * MILLISECOND_PER_ONE_SEC * SECOND_OF_ONE_HOUR + next_days * MILLISECOND_PER_ONE_DAY)))
    }
}
