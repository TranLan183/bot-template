...
function decodeConstructor(abi: any, data: any) {
...
export function decodeData(abi: any, data: string | Buffer): {
    method: string
    types: any[]
    inputs: any[]
    names: any[]
    ...
    if (!result.method) {
        try {
            const decoded = decodeConstructor(abi, data)
            if (decoded) {
                return decoded
            }
        } catch (err) {}
